...
@router.post("/delete_banned_users")
def delete_banned_users(x_cron_secret: str = Header(None), db: Session = Depends(get_db)):
    # Delete banned users after two weeks in the banlist

    if x_cron_secret != settings.CRON_SECRET or not settings.CRON_SECRET:
        logger.warning("Unauthorized attempt to access /delete_banned_users")
        raise Unauthorized("Unauthorized request")

    logger.info("Starting scheduled DB cleanup task for banned users")
