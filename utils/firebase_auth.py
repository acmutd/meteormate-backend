...
if not firebase_admin._apps:
    try:
        ...
        logger.critical(
            f"Failed to initialize Firebase Admin SDK: {str(e)}", stack_info=settings.DEBUG
        )
        raise
...
async def get_current_user(credentials: HTTPAuthorizationCredentials = Depends(security)):
    ...
    try:
        ...
        logger.error(f"Unexpected authentication error: {str(e)}", stack_info=settings.DEBUG)
...
async def get_firebase_user(uid: str):
    try:
        ...
        logger.error(f"Error fetching Firebase user {uid}: {str(e)}", stack_info=settings.DEBUG)
...
async def get_firebase_and_uid(email: str = None, uid: str = None):
    try:
        ...
        logger.error(str(e), stack_info=settings.DEBUG)
